...
async function hitungDanTampilkan() {
    ...
    try {
        ...
        if (window.editId) {
            ...
            response = await fetch(`http://localhost:3000/api/candidates/${window.editId}`, {
                method: 'PUT',
            ...
            response = await fetch('http://localhost:3000/api/candidates', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function hitungDanTampilkan() {
    // ... (logika hitung seperti sebelumnya)
    const results = hitungFuzzyMultiple(calonData);
    if (results.length === numCalon) {
        // Save to DB
        try {
            const response = await fetch('http://localhost:3000/api/candidates', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(results.map(r => ({ ...calonData.find(c => c.nama === r.nama), score: r.score, status: r.status })))
            });
            if (!response.ok) throw new Error('Save failed');
            localStorage.setItem('fuzzyResults', JSON.stringify(results)); // Tetap simpan untuk result.html
            window.location.href = 'result.html';
        } catch (err) {
